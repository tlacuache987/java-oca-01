...
public class League {
	...
	// TODO - Still some problems with displaying League when less (or more) than
	// five teams
	...
	public void setTeamStats(Team[] theTeams, Game[] theGames) {
		...
		for (Game currGame : theGames) {
			...
			if (theResult.isDrawnGame()) {
				currGame.getHomeTeam().incPointsTotal(Settings.DRAWN_GAME_POINTS); // Another way to do it currGame vs.
																					// theResult
	...
	// Should this zero all players first therefore make getAllPlayers() a util
	// method? TODO
	...
	public void showBestPlayersByTeam(Team[] theTeams) {
		...
		for (Team currTeam : theTeams) {
			Arrays.sort(currTeam.getPlayerArray(),
					(p1, p2) -> Double.valueOf(p2.getGoalsScored()).compareTo(Double.valueOf(p1.getGoalsScored())));
	...
	public IDisplayDataItem[][] getLeagueDataGrid(Game[] theGames, Team[] theTeams) {
		...
		// extra columns on right for Points and Goals. Also extra column on top for
		// list of
		// Teams.
		...
		// Add each row of Games for each home team (note all Team IDs will be set by
		// previous for loop
	...
	public void outputTextLeagueGrid(IDisplayDataItem[][] dataGrid) {
		...
		// It is simple to iterate throught the dataGrid and get each item. However,
		// this outputTextLeagueGrid() method
		// will output a grid using monospaced text and therefore needs to calculate how
		// wide to make each column.
		...
		// Create horizontal line of correct length. One extra added to line length for
		// last vertical bar
		...
		for (IDisplayDataItem[] theRow : dataGrid) {
			...
			for (int j = 0; j < theRow.length; j++) {
				...
				System.out.print("| " + theRow[j].getDisplayDetail()
