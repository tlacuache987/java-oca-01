...
public class Game implements IDisplayDataItem {
	...
	public void playGame() {
		...
		currEvent.setThePlayer(
				currEvent.getTheTeam().getPlayerArray()[(int) (Math.random() * this.homeTeam.getPlayerArray().length)]);
		...
		for (int i = 1; i <= Settings.GAME_LENGTH; i++) {
			...
			if (Math.random() > Settings.GAME_EVENT_FREQUENCY) {
				...
				// This is the code that selects an event randomly from the possible set of
				// events.
				/*
				 * GameEvent[] possEvents = currEvent.getNextEvents(); currEvent =
				 * possEvents[(int) (Math.random() (possEvents.length))];
				 */
				// Need to replace with

				currEvent = currTeam.getNextPlayAttempt(currEvent);

				// This is now the new currEvent so need to know if should change player and
				// team
				// currEvent.setTheTeam(currEvent.changeTeam()?getOtherTeam(currTeam):
				// currTeam);

				// TODO for Shoot, ball should not move forward - should be same as end of prev
				// event
				...
				// currPlayer = event1.changePlayer()?currTeam.getPlayerArray()[playerIndex =
				// (playerIndex + 1 +
				// ((int)(Math.random()*(playerArrLength-1))))%playerArrLength]:currPlayer; //
				// Always change
				...
				ArrayList<Player> currPlayerList = new ArrayList(
						Arrays.asList(currEvent.getTheTeam().getPlayerArray()));
				currPlayerList.remove(currPlayer);
				currEvent.setThePlayer(currEvent.changePlayer() ?
				...
				// currEvent.getTheTeam().getPlayerArray().length)]:
						...
						// currEvent.changePlayer()?currTeam.getPlayerArray()[playerIndex = (playerIndex
						// + 1 + ((int)(Math.random()*(playerArrLength-1))))%playerArrLength]:
						currPlayer);
	...
	public String getDescription(boolean showEvents) {
		...
		if (showEvents) {
			for (GameEvent currEvent : this.getEvents()) {
				returnString.append(currEvent.getBallPos() + " : " + currEvent + "after " + currEvent.getTheTime()
						+ " mins by " + currEvent.getThePlayer().getPlayerName() + " of "
